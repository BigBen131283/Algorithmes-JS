function sortedStudents(arr) {
  ...
  for(let i = 0 ; i < arr.length; i++) {
    arr[i].moyenne = moyenne(arr[i].grades)
  }

  arr.sort((a, b) => b.moyenne - a.moyenne) 
  
  for(let i = 0 ; i < arr.length; i++){
    delete arr[i].moyenne
  }
...
function moyenne(notes) {
  ...
  return notes.reduce((acc, currentValue) => acc + currentValue, initialValue)/notes.length

}

function sortedStudentsCorrection(arr) {
  ...
  return arr.sort((a,b) => {
    return (
      b.grades.reduce((acc, cur) => acc + cur) / b.grades.length - a.grades.reduce((acc, cur) => acc + cur) / a.grades.length
    )
  })
